  ...
  function verdict() {
    if (score >= 3)
      return {
        label: "탐구형에 가깝습니다",
        detail:
          "직접 검증·설계하는 탐구형 연구질문에 가까운 편입니다. 이 상태로 다음 단계(선행연구 조사)로 넘어가도 괜찮습니다.",
      };
    if (score === 2)
      return {
        label: "탐구형과 조사형 사이",
        detail:
          "조금만 더 다듬으면 탐구형이 될 수 있어요. \"왜?\"를 한 번 더 물어보거나, 아직 답이 안 나온 세부 조건을 하나 넣어보세요.",
      };
    return {
      label: "아직 조사형에 가깝습니다",
      detail:
        "지금 상태로는 자료를 찾아 정리하는 선에서 머물 가능성이 큽니다. 위쪽 \"한 걸음 더 들어가는 세 가지 사고 습관\" 부분을 다시 짚어보세요.",
    };
  }
